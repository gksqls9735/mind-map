...
// contextBridge.exposeInMainWorld(apiKey, apiObject) > 웹 페이지의 window 객체에 안전하게 API를 노출
// electronAPI > apiKey로 웹 페이지에서 사용할 API의 이름, window.electronAPI로 접근

// ipcRenderer > 웹 페이지에서 메인 프로세서로 메세지를 보내거나 받을 때 사용
contextBridge.exposeInMainWorld('electronAPI', {
   ...
   */
  saveFile: (data: { nodes: any[]; edges: any[] }) => ipcRenderer.send('save-file', data),
   ...
   */
  onFileLoaded: (callback: (data: { nodes: any[], edges: any[] }) => void) => 
